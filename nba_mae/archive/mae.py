...
# Specify the folder path here for id conversions
...
hf_api = HfApi(
    ...
    token="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", # Token is not persisted on the machine.
)

login(token="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", add_to_git_credential=True)
...
trainer.push_to_hub(token="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")


model_name = model_ckpt.split("/")[-1]
new_model_name = datetime.now().strftime("BC_%H:%M:%S_%d_%b")
